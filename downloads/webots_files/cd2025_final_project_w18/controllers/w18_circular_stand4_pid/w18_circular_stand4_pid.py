...
class PID:
    def __init__(self, kp, ki, kd, windup_guard=0.4):
        self.kp = kp
        self.ki = ki
        self.kd = kd
    ...
    def update(self, error, dt):
        ...
        self.int_error = max(min(self.int_error, self.windup_guard), -self.windup_guard)
...
def goto_point_and_face_to(
    ...
    fast_speed=1.7, slow_speed=0.3, slow_dist=0.7, goal_threshold=0.23,
    min_speed=0.07, min_omega_deg=2.5
):
    """
    ...
    # PID for heading control（角度誤差控制旋轉，抑制來回抖動）
    heading_pid = PID(kp=1.0, ki=0.0, kd=0.24, windup_guard=0.6)
    # PID for forward distance control（前進速度穩定控制，不會抖動/衝過頭）
    forward_pid = PID(kp=1.05, ki=0.04, kd=0.25, windup_guard=0.35)
    ...
    while True:
        ...
        if abs(rotate_angle) < min_omega_deg:
            ...
            omega = heading_pid.update(math.radians(rotate_angle), dt)
            omega = max(min(omega, 2.2), -2.2)
        # 動態調速
        ...
        if abs(dist) < min_speed:
            ...
            # 前進方向PID（只考慮前進分量，不考慮側向分量）
            forward_v = forward_pid.update(dist, dt)
            forward_v = max(min(forward_v, speed), -speed)
            vx_world = (dx / (dist + 1e-9)) * forward_v
            vy_world = (dy / (dist + 1e-9)) * forward_v * 0.18  # 側向分量再降權
        # 轉成本體座標速度
        ...
        # 若距離已經開始增加，強制停車（避免震盪）
        if dist > prev_dist + 0.012 and dist < 0.32:
    ...
    while True:
        ...
        if abs(error) < tolerance:
            break
        # == PID 控制旋轉 ==
        omega = heading_pid.update(math.radians(error), dt)
