...
def goto_point_and_face_to(robot, gps, imu, wheels, x_goal, y_goal, face_to=(0.0, 0.0), 
                           fast_speed=1.2, slow_speed=0.3, slow_dist=0.5, goal_threshold=0.15):
    """
    控制 youBot 前進到 (x_goal, y_goal)，到達後車頭朝向 face_to (x, y)。
    前進過程先用較快速度，接近目標時自動減速。
    角度控制用向量夾角計算。
    """
